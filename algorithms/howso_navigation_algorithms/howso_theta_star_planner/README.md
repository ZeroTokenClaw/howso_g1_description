# Howso Theta Star Planner
The Theta Star Planner is a global planning plugin meant to be used with the Howso Planner Server. The `howso_theta_star_planner` implements a highly optimized version of the Theta\* Planner (specifically the [Lazy Theta\* P variant](http://idm-lab.org/bib/abstracts/papers/aaai10b.pdf)) meant to plan any-angle paths using A\*. The planner supports differential-drive and omni-directional robots.

See its [Configuration Guide Page](https://docs.howso.org/configuration/packages/configuring-thetastar.html) for additional parameter descriptions.
...
planner_server:
  ros__parameters:
    planner_plugin_types: ["howso_theta_star_planner::ThetaStarPlanner"]
