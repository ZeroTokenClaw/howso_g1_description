...
#include "howso_theta_star_planner/theta_star_planner.hpp"
...
{
  RCLCPP_INFO(logger_, "CleaningUp plugin %s of type howso_theta_star_planner", name_.c_str());
  planner_.reset();
}

void ThetaStarPlanner::activate()
{
  RCLCPP_INFO(logger_, "Activating plugin %s of type howso_theta_star_planner", name_.c_str());
  param_handler_->activate();
}

void ThetaStarPlanner::deactivate()
{
  RCLCPP_INFO(logger_, "Deactivating plugin %s of type howso_theta_star_planner", name_.c_str());
  auto node = parent_node_.lock();
  ...
  std::function<bool()> cancel_checker)
{
  ...
  std::unique_lock<howso_costmap_2d::Costmap2D::mutex_t> lock(*(planner_->costmap_->getMutex()));
  ...
  if (planner_->costmap_->getCost(mx_goal, my_goal) == howso_costmap_2d::LETHAL_OBSTACLE) {
    throw howso_core::GoalOccupied(
            "Goal Coordinates of(" + std::to_string(goal.pose.position.x) + ", " +
  ...
  std::function<bool()> cancel_checker)
{
  ...
  if (planner_->isUnsafeToPlan()) {
    ...
    throw howso_core::PlannerException("Either of the start or goal pose are an obstacle! ");
  } else if (planner_->generatePath(path, cancel_checker)) {
    global_path = linearInterpolation(path, planner_->costmap_->getResolution());
  } else {
    global_path.poses.clear();
    throw howso_core::NoValidPathCouldBeFound("Could not generate path between the given poses");
  }
  global_path.header.stamp = clock_->now();
  global_path.header.frame_id = global_frame_;
}
...
PLUGINLIB_EXPORT_CLASS(howso_theta_star_planner::ThetaStarPlanner, howso_core::GlobalPlanner)
