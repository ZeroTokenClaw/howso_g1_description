...
class test_theta_star : public howso_theta_star_planner::ThetaStar
{
public:
  explicit test_theta_star(howso_theta_star_planner::Parameters * params)
  : ThetaStar(params) {}
...
TEST(ThetaStarTest, test_theta_star) {
  auto node = std::make_shared<howso::LifecycleNode>("ThetaStarTestNode");
  auto plugin_name = std::string("test");
  auto param_handler = std::make_unique<howso_theta_star_planner::ParameterHandler>(
  ...
  planner_->costmap_ = new howso_costmap_2d::Costmap2D(50, 50, 1.0, 0.0, 0.0, 0);
  for (int i = 7; i <= 14; i++) {
    for (int j = 7; j <= 14; j++) {
...
TEST(ThetaStarPlanner, test_theta_star_planner) {
  howso::LifecycleNode::SharedPtr life_node =
    std::make_shared<howso::LifecycleNode>("ThetaStarPlannerTest");
  ...
  goal = start;
  viapoint = start;
  auto planner_2d = std::make_unique<howso_theta_star_planner::ThetaStarPlanner>();
...
TEST(ThetaStarPlanner, test_theta_star_reconfigure)
{
  ...
  auto planner = std::make_unique<howso_theta_star_planner::ThetaStarPlanner>();
  try {
