...
Howso Navigation Algorithms（华苏导航算法）是面向机器人仿真、路径规划和自主导航验证的算法参考模块。本目录基于 ROS Navigation2 选取与仿真平台相关性较高的规划、控制、代价地图和接口包，用于支撑后续的导航算法评估、仿真场景验证和系统集成设计。
...
目录内部的 `nav2_*` 包名保留上游 ROS 2 包名，便于保持源码引用、构建关系和许可证追溯的一致性。
