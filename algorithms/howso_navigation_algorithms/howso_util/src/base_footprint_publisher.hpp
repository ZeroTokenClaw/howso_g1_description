 ...
 */
class BaseFootprintPublisherListener : public howso::TransformListener
{
public:
  //  nosemgrep
  BaseFootprintPublisherListener(tf2::BufferCore & buffer, bool spin_thread, rclcpp::Node & node)
  : howso::TransformListener(buffer, spin_thread)
  {
    base_link_frame_ = howso::declare_or_get_parameter(
      &node, "base_link_frame", std::string("base_link"));
    base_footprint_frame_ = howso::declare_or_get_parameter(
      &node, "base_footprint_frame", std::string("base_footprint"));
    tf_broadcaster_ = howso::create_transform_broadcaster(&node);
  }
...
/**
 * @class howso_util::BaseFootprintPublisher
...
public:
  ...
  : Node("base_footprint_publisher", options)
  {
    ...
    tf_buffer_ = howso::create_transform_buffer(this);
