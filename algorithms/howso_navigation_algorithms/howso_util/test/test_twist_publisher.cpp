...
#include "howso_util/twist_publisher.hpp"
...
TEST(TwistPublisher, Unstamped)
{
  ...
  auto pub_node = std::make_shared<howso::LifecycleNode>("pub_node", "");
  pub_node->configure();
  pub_node->declare_parameter("enable_stamped_cmd_vel", rclcpp::ParameterValue(false));
  auto vel_publisher = std::make_unique<howso_util::TwistPublisher>(
  ...
  auto sub_node = std::make_shared<howso::LifecycleNode>("sub_node", "");
  sub_node->configure();
  sub_node->activate();
...
TEST(TwistPublisher, Stamped)
{
  ...
  pub_node->configure();
  auto vel_publisher = std::make_unique<howso_util::TwistPublisher>(pub_node, "cmd_vel");
  ASSERT_EQ(vel_publisher->get_subscription_count(), 0);
  EXPECT_FALSE(vel_publisher->is_activated());
  pub_node->activate();
  EXPECT_TRUE(vel_publisher->is_activated());
