...
TEST(PathUtilsTest, EmptyPath)
{
  ...
  auto result = howso_util::distance_from_path(empty_path, robot_pose);
...
TEST_F(CuttingCornerTest, TrajectoryCutsCorner)
{
  for (size_t i = 0; i < robot_trajectory.size(); ++i) {
    ...
    auto result = howso_util::distance_from_path(target_path, robot_pose);
...
TEST_F(CuttingCornerWindowedTest, WindowedSearch)
{
  ...
  for (size_t i = 0; i < robot_trajectory.size(); ++i) {
    const auto & robot_pose = robot_trajectory[i];
    auto result = howso_util::distance_from_path(
...
TEST(PathUtilsWindowedTest, EdgeCases)
{
  ...
  auto result = howso_util::distance_from_path(test_path, robot_pose, 0, 5.0);
  EXPECT_NEAR(std::abs(result.distance), 5.0, 0.01);
}

TEST(PathUtilsTest, FourArgEmptyPath)
{
  nav_msgs::msg::Path empty_path;
  geometry_msgs::msg::Pose robot_pose = createPose(1.0, 1.0);
  auto result = howso_util::distance_from_path(empty_path, robot_pose, 0, 5.0);
...
TEST(PathUtilsTest, FourArgSinglePointPath)
{
  ...
  auto result = howso_util::distance_from_path(path, robot_pose, 0, 5.0);
  // Distance between (5,7) and (2,3) is 5.0
  EXPECT_NEAR(result.distance, 5.0, 1e-6);
  EXPECT_EQ(result.closest_segment_index, 0);
}

TEST(PathUtilsTest, FourArgThrowsOnStartIndexOutOfBounds)
{
  nav_msgs::msg::Path path;
  ...
  EXPECT_THROW(
    howso_util::distance_from_path(path, robot_pose, 3, 5.0),  // 3 >= 3
    std::runtime_error);

  // start_index is way out of bounds
  EXPECT_THROW(
    howso_util::distance_from_path(path, robot_pose, 100, 5.0),  // 100 >= 3
    std::runtime_error);
}
...
TEST(TransformPathTest, SuccessfulTransform)
{
  ...
  howso::TransformBuffer::SharedPtr tf_buffer_ =
    howso::create_transform_buffer(node_);
  howso::TransformListener::SharedPtr tf_listener_ =
    howso::create_transform_listener(*tf_buffer_, node_);
  ...
  EXPECT_TRUE(howso_util::transformPathInTargetFrame(
...
TEST(TransformPathTest, MissingTransform)
{
  ...
  EXPECT_FALSE(howso_util::transformPathInTargetFrame(
...
TEST(UtilsTests, FindPathInversionTest)
{
  ...
  EXPECT_EQ(howso_util::findFirstPathConstraint(path, true, 0.0), 10u);

  // To short to process
  path.poses.erase(path.poses.begin(), path.poses.begin() + 7);
  EXPECT_EQ(howso_util::findFirstPathConstraint(path, true, 0.0), 3u);
  ...
  EXPECT_EQ(howso_util::findFirstPathConstraint(path, true, 0.0), 11u);
  ...
  path.poses.push_back(last_pose);
  EXPECT_EQ(howso_util::findFirstPathConstraint(path, true, 1.57), 10u);
  ...
  EXPECT_EQ(howso_util::findFirstPathConstraint(path, true, 0.5), path.poses.size());
  ...
  EXPECT_EQ(howso_util::findFirstPathConstraint(path, true, 1.5), 7u);
  ...
  EXPECT_EQ(howso_util::findFirstPathConstraint(path, true, 0.7), 6u);
}
...
TEST(UtilsTests, RemovePosesAfterPathInversionTest)
{
  ...
  EXPECT_EQ(howso_util::removePosesAfterFirstConstraint(path, true, 0.0), 0u);
  ...
  EXPECT_EQ(howso_util::removePosesAfterFirstConstraint(path, true, 0.0), 11u);
  // Check to see if removed
  EXPECT_EQ(path.poses.size(), 11u);
  EXPECT_EQ(path.poses.back().pose.position.x, 10);
}

TEST(UtilsTests, IsPathOrGoalUpdatedTest)
{
  ...
  EXPECT_TRUE(howso_util::isPathUpdated(old_path, new_path));
  EXPECT_FALSE(howso_util::isGoalUpdated(old_path, new_path));
  ...
  new_path.poses.push_back(makePose(0.0, 0.0));
  new_path.poses.push_back(makePose(1.0, 1.0));
  EXPECT_FALSE(howso_util::isPathUpdated(old_path, new_path));
  EXPECT_FALSE(howso_util::isGoalUpdated(old_path, new_path));
  ...
  old_path.poses.push_back(makePose(1.0, 1.0));
  new_path.poses.push_back(makePose(0.0, 0.0));
  new_path.poses.push_back(makePose(2.0, 2.0));
  EXPECT_FALSE(howso_util::isPathUpdated(old_path, new_path));
  EXPECT_TRUE(howso_util::isGoalUpdated(old_path, new_path));
}
