...
using howso_util::geometry_utils::euclidean_distance;
using howso_util::geometry_utils::calculate_path_length;
using howso_util::geometry_utils::find_next_matching_goal_in_waypoint_statuses;
...
TEST(GeometryUtils, find_next_matching_goal_in_waypoint_statuses)
{
  ...
  std::vector<howso_msgs::msg::WaypointStatus> waypoint_statuses(nb_waypoints);
  ...
  waypoint_statuses[matching_index].waypoint_status = howso_msgs::msg::WaypointStatus::COMPLETED;
  ASSERT_EQ(
    find_next_matching_goal_in_waypoint_statuses(
      waypoint_statuses,
...
TEST(GeometryUtils, cross_product_2d_basic)
{
  ...
  double result = howso_util::geometry_utils::cross_product_2d(point, start, end);
  ...
  point.y = -2.0;
  // Cross product: (2*-2) - (0*1) = -4
  result = howso_util::geometry_utils::cross_product_2d(point, start, end);
  EXPECT_NEAR(result, -4.0, 1e-6);
