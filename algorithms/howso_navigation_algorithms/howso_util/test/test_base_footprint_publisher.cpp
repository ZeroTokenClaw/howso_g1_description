...
{
  auto node = std::make_shared<howso_util::BaseFootprintPublisher>();
  rclcpp::executors::SingleThreadedExecutor executor;
  executor.add_node(node->get_node_base_interface());
  executor.spin_some();

  auto tf_broadcaster = howso::create_transform_broadcaster(node);
  auto buffer = howso::create_transform_buffer(node);
  auto listener = howso::create_transform_listener(*buffer, node);
