...
void CriticManager::on_configure(
  ...
  std::shared_ptr<howso_costmap_2d::Costmap2DROS> costmap_ros, ParametersHandler * param_handler)
{
  parent_ = parent;
...
void CriticManager::loadCritics()
{
  if (!loader_) {
    loader_ = std::make_unique<pluginlib::ClassLoader<critics::CriticFunction>>(
      "howso_mppi_controller", "mppi::critics::CriticFunction");
  }

  auto node = parent_.lock();
  if (visualize_) {
    critics_effect_pub_ = node->create_publisher<howso_msgs::msg::CriticsStats>(
      "~/critics_stats");
...
void CriticManager::evalTrajectoriesScores(
  CriticData & data)
{
  std::unique_ptr<howso_msgs::msg::CriticsStats> stats_msg;
  if (visualize_) {
    data.trajectories_in_collision.assign(data.costs.size(), false);
    critic_costs_.clear();
    critic_costs_.reserve(critics_.size());
    stats_msg = std::make_unique<howso_msgs::msg::CriticsStats>();
