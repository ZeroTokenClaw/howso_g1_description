...
#include "howso_mppi_controller/optimizer.hpp"
#include "howso_mppi_controller/tools/parameters_handler.hpp"
#include "howso_mppi_controller/controller.hpp"
...
std::shared_ptr<howso_costmap_2d::Costmap2DROS> getDummyCostmapRos()
{
  auto costmap_ros = std::make_shared<howso_costmap_2d::Costmap2DROS>("cost_map_node");
  costmap_ros->on_configure(rclcpp_lifecycle::State{});
  return costmap_ros;
}

std::shared_ptr<howso_costmap_2d::Costmap2D> getDummyCostmap(TestCostmapSettings s)
{
  auto costmap = std::make_shared<howso_costmap_2d::Costmap2D>(
...
std::shared_ptr<howso_costmap_2d::Costmap2DROS> getDummyCostmapRos(TestCostmapSettings s)
{
...
howso::LifecycleNode::SharedPtr
getDummyNode(
  TestOptimizerSettings s, std::vector<std::string> critics,
  std::string node_name = std::string("dummy"))
{
  auto node =
    std::make_shared<howso::LifecycleNode>(node_name, getOptimizerOptions(s, critics));
  return node;
}
...
{
  auto node = std::make_shared<howso::LifecycleNode>(node_name, options);
  return node;
}
  ...
  TParamHandler * params_handler)
{
  ...
  howso::LifecycleNode::WeakPtr weak_ptr_node{node};
...
std::shared_ptr<howso_mppi_controller::MPPIController> getDummyController(
  TNode node, TFBuffer tf_buffer,
  TCostMap costmap_ros)
{
  auto controller = std::make_shared<howso_mppi_controller::MPPIController>();
  howso::LifecycleNode::WeakPtr weak_ptr_node{node};
