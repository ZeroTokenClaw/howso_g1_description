...
void sendTf(
  ...
  howso::TransformBroadcaster::SharedPtr tf_broadcaster,
  howso::LifecycleNode::SharedPtr node, size_t n)
{
  while (--n != 0u) {
 ...
 */
void printMap(const howso_costmap_2d::Costmap2D & costmap)
{
...
void printMapWithTrajectoryAndGoal(
  howso_costmap_2d::Costmap2D & costmap, const TTrajectory & trajectory,
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // create new costmap
  howso_costmap_2d::Costmap2D costmap2d(
 ...
 */
void addObstacle(
  howso_costmap_2d::Costmap2D * costmap, unsigned int upper_left_corner_x,
...
void addObstacle(howso_costmap_2d::Costmap2D * costmap, TestObstaclesSettings s)
{
 ...
 */
template<typename TTrajectory>
bool inCollision(const TTrajectory & trajectory, const howso_costmap_2d::Costmap2D & costmap)
{
  unsigned int point_mx = 0;
  unsigned int point_my = 0;
  ...
  for (size_t i = 0; i < trajectory.rows(); ++i) {
    ...
    if (cost_ > howso_costmap_2d::FREE_SPACE || cost_ == howso_costmap_2d::NO_INFORMATION) {
      return true;
    }
  }
  return false;
...
unsigned char getCost(const howso_costmap_2d::Costmap2D & costmap, double x, double y)
{
  unsigned int point_mx = 0;
  unsigned int point_my = 0;
...
template<typename TTrajectory>
bool isGoalReached(
  const TTrajectory & trajectory, const howso_costmap_2d::Costmap2D & costmap,
  const geometry_msgs::msg::PoseStamped & goal)
{
