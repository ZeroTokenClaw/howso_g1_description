...
- global planner (e.g., `howso_navfn_planner`)
- controller (e.g., path execution controller, e.g `howso_dwb_controller`)
- smoother (e.g., `howso_ceres_costaware_smoother`)
- goal checker (e.g. `simple_goal_checker`)
- behaviors (e.g. `drive_on_heading`)
- progress checker (e.g. `simple_progress_checker`)
- path handler (e.g. `feasible_path_handler`)
