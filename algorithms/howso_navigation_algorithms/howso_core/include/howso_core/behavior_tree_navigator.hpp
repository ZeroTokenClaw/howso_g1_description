...
#ifndef HOWSO_CORE__BEHAVIOR_TREE_NAVIGATOR_HPP_
#define HOWSO_CORE__BEHAVIOR_TREE_NAVIGATOR_HPP_
...
#include "howso_behavior_tree/bt_action_server.hpp"
#include "howso_ros_common/node_utils.hpp"

namespace howso_core
{
...
struct FeedbackUtils
{
  ...
  howso::TransformBuffer::SharedPtr tf;
};
...
public:
  ...
  virtual bool on_configure(
    howso::LifecycleNode::WeakPtr parent_node,
    const std::vector<std::string> & plugin_lib_names,
    const FeedbackUtils & feedback_utils,
    howso_core::NavigatorMuxer * plugin_muxer,
    std::shared_ptr<howso_util::OdomSmoother> odom_smoother) = 0;
...
public:
   ...
   */
  void startNavigating(howso_core::NavigatorBase * navigator)
  {
    std::scoped_lock l(mutex_);
   ...
   */
  void stopNavigating(howso_core::NavigatorBase * navigator)
  {
    std::scoped_lock l(mutex_);
...
protected:
  howso_core::NavigatorBase * current_navigator_{nullptr};
  std::mutex mutex_;
};
...
{
public:
  using Ptr = std::shared_ptr<howso_core::BehaviorTreeNavigator<ActionT>>;
  ...
  bool on_configure(
    ...
    const FeedbackUtils & feedback_utils,
    howso_core::NavigatorMuxer * plugin_muxer,
    std::shared_ptr<howso_util::OdomSmoother> odom_smoother) final
  {
    ...
    auto search_directories = node->declare_or_get_parameter(
      ...
      std::vector<std::string>{howso::get_package_share_directory(
          "howso_bt_navigator") + "/behavior_trees"}
    );
    ...
    bt_action_server_ =
      std::make_unique<howso_behavior_tree::BtActionServer<ActionT, howso::LifecycleNode>>(
      node,
      getName(),
      plugin_lib_names,
  ...
  virtual std::string getDefaultBTFilepath(howso::LifecycleNode::WeakPtr node) = 0;
...
protected:
  ...
  void onCompletion(
    ...
    howso_behavior_tree::BtStatus & final_bt_status)
  {
  ...
  virtual void goalCompleted(
    typename ActionT::Result::SharedPtr result,
    howso_behavior_tree::BtStatus & final_bt_status) = 0;

  /**
   * @param Method to configure resources.
   */
  virtual bool configure(
    howso::LifecycleNode::WeakPtr /*node*/,
    std::shared_ptr<howso_util::OdomSmoother>/*odom_smoother*/)
  {
    return true;
  }
  ...
  std::unique_ptr<howso_behavior_tree::BtActionServer<ActionT, howso::LifecycleNode>>
  bt_action_server_;
...
#endif  // HOWSO_CORE__BEHAVIOR_TREE_NAVIGATOR_HPP_
