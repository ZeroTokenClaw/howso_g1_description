...
#ifndef HOWSO_SMAC_PLANNER__SMAC_PLANNER_2D_IMPL_HPP_
#define HOWSO_SMAC_PLANNER__SMAC_PLANNER_2D_IMPL_HPP_
...
#include "howso_smac_planner/smac_planner_2d.hpp"
...
SmacPlanner2DT<NodeT>::SmacPlanner2DT()
: _a_star(nullptr),
...
template<typename NodeT>
void SmacPlanner2DT<NodeT>::activate()
{
  RCLCPP_INFO(
...
template<typename NodeT>
void SmacPlanner2DT<NodeT>::deactivate()
{
  RCLCPP_INFO(
...
template<typename NodeT>
void SmacPlanner2DT<NodeT>::cleanup()
{
  RCLCPP_INFO(
...
template<typename NodeT>
nav_msgs::msg::Path SmacPlanner2DT<NodeT>::createPlan(
  ...
  std::function<bool()> cancel_checker)
{
  ...
  howso_costmap_2d::Costmap2D * costmap = _costmap;
  ...
  if (_use_final_approach_orientation) {
    if (plan_size == 1) {
      ...
      plan.poses.back().pose.orientation =
        howso_util::geometry_utils::orientationAroundZAxis(theta);
    }
  } else if (plan_size > 0) {
...
template<typename NodeT>
void
SmacPlanner2DT<NodeT>::updateParametersCallback(const std::vector<rclcpp::Parameter> & parameters)
{
  std::lock_guard<std::mutex> lock_reinit(_mutex);
...
#endif  // HOWSO_SMAC_PLANNER__SMAC_PLANNER_2D_IMPL_HPP_
