...
#ifndef HOWSO_SMAC_PLANNER__UTILS_HPP_
#define HOWSO_SMAC_PLANNER__UTILS_HPP_
...
inline geometry_msgs::msg::Pose getWorldCoords(
  const float & mx, const float & my, const howso_costmap_2d::Costmap2D * costmap)
{
...
*/
inline double findCircumscribedCost(std::shared_ptr<howso_costmap_2d::Costmap2DROS> costmap)
{
  double result = -1.0;
  std::vector<std::shared_ptr<howso_costmap_2d::Layer>>::iterator layer;

  // check if the costmap has an inflation layer
  const auto inflation_layer = howso_costmap_2d::InflationLayerInterface::getInflationLayer(costmap);
...
#endif  // HOWSO_SMAC_PLANNER__UTILS_HPP_
