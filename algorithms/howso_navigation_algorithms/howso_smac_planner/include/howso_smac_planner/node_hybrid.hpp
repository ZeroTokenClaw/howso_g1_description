...
#ifndef HOWSO_SMAC_PLANNER__NODE_HYBRID_HPP_
#define HOWSO_SMAC_PLANNER__NODE_HYBRID_HPP_
...
#include "howso_smac_planner/constants.hpp"
#include "howso_smac_planner/types.hpp"
#include "howso_smac_planner/collision_checker.hpp"
#include "howso_smac_planner/costmap_downsampler.hpp"
#include "howso_smac_planner/obstacle_heuristic.hpp"
#include "howso_smac_planner/distance_heuristic.hpp"
...
/**
 * @struct howso_smac_planner::HybridMotionTable
 * @brief A table of motion primitives and related functions
 */
struct HybridMotionTable
{
  /**
   * @brief A constructor for howso_smac_planner::HybridMotionTable
   */
  HybridMotionTable() {}
...
/**
 * @class howso_smac_planner::NodeHybrid
...
public:
  ...
  using Coordinates = howso_smac_planner::Coordinates;
  typedef std::vector<Coordinates> CoordinateVector;

  struct NodeContext
  {
    /**
     * @brief A constructor for howso_smac_planner::NodeContext
     */
    NodeContext()
    {
  ...
  /**
   * @brief A constructor for howso_smac_planner::NodeHybrid
   * @param index The index of this node for self-reference
   */
  explicit NodeHybrid(const uint64_t index, NodeContext * ctx);

  /**
   * @brief A destructor for howso_smac_planner::NodeHybrid
   */
  ~NodeHybrid();
  ...
  void getNeighbors(
    ...
    howso_smac_planner::NodeHybrid * &)> & validity_checker,
    GridCollisionChecker * collision_checker,
    const bool & traverse_unknown,
...
#endif  // HOWSO_SMAC_PLANNER__NODE_HYBRID_HPP_
