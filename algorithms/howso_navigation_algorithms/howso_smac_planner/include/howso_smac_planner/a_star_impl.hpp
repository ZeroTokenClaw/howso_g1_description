...
#ifndef HOWSO_SMAC_PLANNER__A_STAR_IMPL_HPP_
#define HOWSO_SMAC_PLANNER__A_STAR_IMPL_HPP_
...
bool AStarAlgorithm<NodeT>::areInputsValid()
{
  ...
  if (_goal_manager.getGoalsSet().empty()) {
    throw howso_core::GoalOccupied("Goal was in lethal cost");
  }
  ...
  std::vector<std::tuple<float, float, float>> * expansions_log)
{
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    if (iterations % _terminal_checking_interval == 0) {
      if (cancel_checker()) {
        throw howso_core::PlannerCancelled("Planner was cancelled");
      }
...
#endif  // HOWSO_SMAC_PLANNER__A_STAR_IMPL_HPP_
