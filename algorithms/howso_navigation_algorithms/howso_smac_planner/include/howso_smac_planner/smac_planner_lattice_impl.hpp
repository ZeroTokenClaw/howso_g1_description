...
#ifndef HOWSO_SMAC_PLANNER__SMAC_PLANNER_LATTICE_IMPL_HPP_
#define HOWSO_SMAC_PLANNER__SMAC_PLANNER_LATTICE_IMPL_HPP_
...
#include "howso_smac_planner/smac_planner_lattice.hpp"
#include "howso_ros_common/tf2_factories.hpp"

// #define BENCHMARK_TESTING

namespace howso_smac_planner
{
...
SmacPlannerLatticeT<NodeT>::SmacPlannerLatticeT()
: _a_star(nullptr),
...
template<typename NodeT>
void SmacPlannerLatticeT<NodeT>::configure(
  const howso::LifecycleNode::WeakPtr & parent,
  std::string name, howso::TransformBuffer::SharedPtr/*tf*/,
  std::shared_ptr<howso_costmap_2d::Costmap2DROS> costmap_ros)
{
  _node = parent;
  ...
  bool smooth_path = node->declare_or_get_parameter(name + ".smooth_path", true);

  // Default to a well rounded model: 16 bin, 0.4m turning radius, ackermann model
  _search_info.lattice_filepath = node->declare_or_get_parameter(
  ...
  _search_info.change_penalty =
    node->declare_or_get_parameter(name + ".change_penalty", 0.05);
  _search_info.non_straight_penalty =
    node->declare_or_get_parameter(name + ".non_straight_penalty", 1.05);
  ...
  _search_info.rotation_penalty =
    node->declare_or_get_parameter(name + ".rotation_penalty", 5.0);
  ...
  _search_info.allow_reverse_expansion =
    node->declare_or_get_parameter(name + ".allow_reverse_expansion", false);
  ...
  if (_goal_heading_mode == GoalHeadingMode::UNKNOWN) {
    ...
    throw howso_core::PlannerException(error_msg);
  }
...
template<typename NodeT>
void SmacPlannerLatticeT<NodeT>::activate()
{
  RCLCPP_INFO(
...
template<typename NodeT>
void SmacPlannerLatticeT<NodeT>::deactivate()
{
  RCLCPP_INFO(
...
template<typename NodeT>
void SmacPlannerLatticeT<NodeT>::cleanup()
{
  RCLCPP_INFO(
  ...
  std::function<bool()> cancel_checker)
{
  ...
  std::unique_lock<howso_costmap_2d::Costmap2D::mutex_t> lock(*(_costmap->getMutex()));
  ...
  if (!_costmap->worldToMapContinuous(
      ...
      my_start))
  {
    throw howso_core::StartOutsideMapBounds(
  ...
  if (!_costmap->worldToMapContinuous(
      ...
      my_goal))
  {
    throw howso_core::GoalOutsideMapBounds(
      ...
      _tolerance / static_cast<float>(_costmap->getResolution()), cancel_checker, expansions.get()))
  {
    ...
    if (num_iterations == 1) {
      throw howso_core::StartOccupied("Start occupied");
    }

    if (num_iterations < _a_star->getMaxIterations()) {
      throw howso_core::NoValidPathCouldBeFound("no valid path found");
    } else {
      throw howso_core::PlannerTimedOut("exceeded maximum iterations");
    }
  }
...
#endif  // HOWSO_SMAC_PLANNER__SMAC_PLANNER_LATTICE_IMPL_HPP_
