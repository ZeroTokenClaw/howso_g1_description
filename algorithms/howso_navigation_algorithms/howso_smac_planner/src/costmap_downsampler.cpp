  ...
  const bool & use_min_cost_neighbor)
{
  ...
  _downsampled_costmap = std::make_unique<howso_costmap_2d::Costmap2D>(
    _downsampled_size_x, _downsampled_size_y, _downsampled_resolution,
    _costmap->getOriginX(), _costmap->getOriginY(), UNKNOWN_COST);

  if (!node.expired()) {
    _downsampled_costmap_pub = std::make_unique<howso_costmap_2d::Costmap2DPublisher>(
...
howso_costmap_2d::Costmap2D * CostmapDownsampler::downsample(
  const unsigned int & downsampling_factor)
{
  _downsampling_factor = downsampling_factor;
  updateCostmapSize();
