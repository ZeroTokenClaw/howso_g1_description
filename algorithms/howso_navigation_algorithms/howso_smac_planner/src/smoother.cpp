  ...
  const std::vector<geometry_msgs::msg::Point> & footprint)
{
  ...
  std::vector<PathSegment> path_segments = howso_util::findDirectionalPathSegments(
    path,
    is_holonomic_);
  ...
  const std::vector<geometry_msgs::msg::Point> & footprint)
{
  ...
  while (change >= tolerance_) {
    ...
    if (its >= max_its_) {
      ...
      path = last_path;
      howso_util::updateApproximatePathOrientations(path, reversing_segment, is_holonomic_);
      return false;
    }
    ...
    howso_util::updateApproximatePathOrientations(
    ...
    if (costmap) {
      if (!footprint.empty()) {
        footprint_checker_.setCostmap(
          const_cast<howso_costmap_2d::Costmap2D *>(costmap));
      }
