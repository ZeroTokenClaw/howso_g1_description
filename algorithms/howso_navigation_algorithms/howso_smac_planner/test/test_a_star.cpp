...
#include "howso_costmap_2d/costmap_subscriber.hpp"
#include "howso_ros_common/lifecycle_node.hpp"
#include "howso_smac_planner/node_hybrid.hpp"
#include "howso_smac_planner/node_lattice.hpp"
#include "howso_smac_planner/a_star.hpp"
#include "howso_smac_planner/collision_checker.hpp"
#include "howso_ros_common/node_utils.hpp"

TEST(AStarTest, test_a_star_2d)
{
  auto lnode = std::make_shared<howso::LifecycleNode>("test");
  howso_smac_planner::SearchInfo info;
  howso_smac_planner::AStarAlgorithm<howso_smac_planner::Node2D> a_star(
    howso_smac_planner::MotionModel::TWOD, info);
  ...
  howso_costmap_2d::Costmap2D * costmapA =
    new howso_costmap_2d::Costmap2D(100, 100, 0.1, 0.0, 0.0, 0);
  ...
  auto costmap_ros = std::make_shared<howso_costmap_2d::Costmap2DROS>();
  ...
  *costmap = *costmapA;
  ...
  std::unique_ptr<howso_smac_planner::GridCollisionChecker> checker =
    std::make_unique<howso_smac_planner::GridCollisionChecker>(costmap_ros, 1, lnode);
  checker->setFootprint(howso_costmap_2d::Footprint(), true, 0.0);
  a_star.setCollisionChecker(checker.get());
  a_star.setStart(20u, 20u, 0);
  a_star.setGoal(80u, 80u, 0);
  howso_smac_planner::Node2D::CoordinateVector path;
  ...
  howso_smac_planner::AStarAlgorithm<howso_smac_planner::Node2D> a_star_2(
    howso_smac_planner::MotionModel::TWOD, info);
  a_star_2.initialize(
    false, max_iterations, it_on_approach, terminal_checking_interval,
  ...
  howso_smac_planner::AnalyticExpansion<howso_smac_planner::Node2D> expander(
    howso_smac_planner::MotionModel::TWOD, info, false, 1);

  auto analytic_expansion_nodes =
    howso_smac_planner::AnalyticExpansion<howso_smac_planner::Node2D>::AnalyticExpansionNodes();
  ...
  howso_smac_planner::Node2D * start = nullptr;
  EXPECT_EQ(
...
TEST(AStarTest, test_a_star_se2)
{
  ...
  howso_smac_planner::AStarAlgorithm<howso_smac_planner::NodeHybrid> a_star(
    howso_smac_planner::MotionModel::DUBIN, info);
  int max_iterations = 10000;
  ...
  *costmap = *costmapA;
  ...
  std::unique_ptr<howso_smac_planner::GridCollisionChecker> checker =
    std::make_unique<howso_smac_planner::GridCollisionChecker>(costmap_ros, size_theta, lnode);
  checker->setFootprint(howso_costmap_2d::Footprint(), true, 0.0);
  ...
  howso_smac_planner::NodeHybrid::CoordinateVector path;
...
TEST(AStarTest, test_a_star_analytic_expansion)
{
  ...
  info.cost_penalty = 1.7;
  howso_smac_planner::AStarAlgorithm<howso_smac_planner::NodeHybrid> a_star(
    howso_smac_planner::MotionModel::REEDS_SHEPP, info);
  int max_iterations = 10000;
  float tolerance = 10.0;
  int it_on_approach = 10;
  int terminal_checking_interval = 5000;
...
TEST(AStarTest, test_a_star_lattice)
{
  ...
  info.lattice_filepath =
    howso::get_package_share_directory("howso_smac_planner") +
  ...
  howso_smac_planner::AStarAlgorithm<howso_smac_planner::NodeLattice> a_star(
    howso_smac_planner::MotionModel::STATE_LATTICE, info);
  int max_iterations = 10000;
  float tolerance = 10.0;
  int terminal_checking_interval = 5000;
  double max_planning_time = 120.0;
  ...
  howso_costmap_2d::Costmap2D * costmapA =
    new howso_costmap_2d::Costmap2D(100, 100, 0.05, 0.0, 0.0, 0);
  // island in the middle of lethal cost to cross
  ...
  *costmap = *costmapA;
  ...
  howso_smac_planner::NodeLattice::CoordinateVector path;
  EXPECT_TRUE(a_star.createPath(path, num_it, tolerance, dummy_cancel_checker));
...
TEST(AStarTest, test_goal_heading_mode)
{
  ...
  *costmap = *costmapA;
  ...
  EXPECT_THROW(
    a_star.setGoal(
      80u, 80u, 40u,
      howso_smac_planner::GoalHeadingMode::BIDIRECTIONAL),
    std::runtime_error);
  a_star.setStart(10u, 10u, 0u);
  a_star.setGoal(80u, 80u, 40u, howso_smac_planner::GoalHeadingMode::BIDIRECTIONAL);
  howso_smac_planner::NodeHybrid::CoordinateVector path;
  std::unique_ptr<std::vector<std::tuple<float, float, float>>> expansions = nullptr;
  expansions = std::make_unique<std::vector<std::tuple<float, float, float>>>();
  ...
  a_star.setStart(10u, 10u, 0u);
  a_star.setGoal(
    80u, 80u, 40u, howso_smac_planner::GoalHeadingMode::ALL_DIRECTION,
  ...
  EXPECT_THROW(
    a_star.setGoal(
      80u, 80u, 10u,
      howso_smac_planner::GoalHeadingMode::UNKNOWN), std::runtime_error);
  delete costmapA;
}

TEST(AStarTest, test_constants)
{
  howso_smac_planner::MotionModel mm = howso_smac_planner::MotionModel::UNKNOWN;  // unknown
  EXPECT_EQ(howso_smac_planner::toString(mm), std::string("Unknown"));
  mm = howso_smac_planner::MotionModel::TWOD;  // 2d
  EXPECT_EQ(howso_smac_planner::toString(mm), std::string("2D"));
  mm = howso_smac_planner::MotionModel::DUBIN;  // dubin
  EXPECT_EQ(howso_smac_planner::toString(mm), std::string("Dubin"));
  mm = howso_smac_planner::MotionModel::REEDS_SHEPP;  // reeds-shepp
  EXPECT_EQ(howso_smac_planner::toString(mm), std::string("Reeds-Shepp"));

  howso_smac_planner::GoalHeadingMode gh = howso_smac_planner::GoalHeadingMode::UNKNOWN;
  EXPECT_EQ(howso_smac_planner::toString(gh), std::string("Unknown"));
  gh = howso_smac_planner::GoalHeadingMode::DEFAULT;  // default
  EXPECT_EQ(howso_smac_planner::toString(gh), std::string("DEFAULT"));
  gh = howso_smac_planner::GoalHeadingMode::BIDIRECTIONAL;  // bidirectional
  EXPECT_EQ(howso_smac_planner::toString(gh), std::string("BIDIRECTIONAL"));
  gh = howso_smac_planner::GoalHeadingMode::ALL_DIRECTION;  // all_direction
  EXPECT_EQ(howso_smac_planner::toString(gh), std::string("ALL_DIRECTION"));

  EXPECT_EQ(
    howso_smac_planner::fromString(
      "2D"), howso_smac_planner::MotionModel::TWOD);
  EXPECT_EQ(howso_smac_planner::fromString("DUBIN"), howso_smac_planner::MotionModel::DUBIN);
  EXPECT_EQ(
    howso_smac_planner::fromString(
      "REEDS_SHEPP"), howso_smac_planner::MotionModel::REEDS_SHEPP);
  EXPECT_EQ(howso_smac_planner::fromString("NONE"), howso_smac_planner::MotionModel::UNKNOWN);

  EXPECT_EQ(
    howso_smac_planner::fromStringToGH(
      "DEFAULT"), howso_smac_planner::GoalHeadingMode::DEFAULT);
  EXPECT_EQ(
    howso_smac_planner::fromStringToGH(
      "BIDIRECTIONAL"), howso_smac_planner::GoalHeadingMode::BIDIRECTIONAL);
  EXPECT_EQ(
    howso_smac_planner::fromStringToGH(
      "ALL_DIRECTION"), howso_smac_planner::GoalHeadingMode::ALL_DIRECTION);
  EXPECT_EQ(
    howso_smac_planner::fromStringToGH("NONE"), howso_smac_planner::GoalHeadingMode::UNKNOWN);
}
