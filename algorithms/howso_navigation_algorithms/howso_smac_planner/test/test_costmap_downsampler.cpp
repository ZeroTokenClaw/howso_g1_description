...
{
  howso::LifecycleNode::SharedPtr node = std::make_shared<howso::LifecycleNode>(
    "CostmapDownsamplerTest");
  howso_smac_planner::CostmapDownsampler downsampler;
  ...
  howso_costmap_2d::Costmap2D * downsampledCostmapA = downsampler.downsample(2);
  ...
  howso_costmap_2d::Costmap2D costmapB(4, 4, 0.10, 0.0, 0.0, 0);

  // downsample it
  downsampler.on_configure(node, "map", "unused_topic", &costmapB, 4);
  downsampler.on_activate();
  howso_costmap_2d::Costmap2D * downsampledCostmapB = downsampler.downsample(4);
