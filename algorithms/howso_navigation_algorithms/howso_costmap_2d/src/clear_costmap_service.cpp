...
using std::any_of;
using ClearExceptRegion = howso_msgs::srv::ClearCostmapExceptRegion;
using ClearAroundRobot = howso_msgs::srv::ClearCostmapAroundRobot;
using ClearAroundPose = howso_msgs::srv::ClearCostmapAroundPose;
using ClearEntirely = howso_msgs::srv::ClearEntireCostmap;
...
void ClearCostmapService::validatePlugins(
  ...
  const std::string & operation_name) const
{
  ...
  validatePlugins(plugins, layers, invalid_plugins);
  ...
  if (!invalid_plugins.empty()) {
    ...
    RCLCPP_ERROR(
      logger_,
      "Failed to %s: %zu invalid plugin(s) out of %zu requested. No layers were cleared.",
