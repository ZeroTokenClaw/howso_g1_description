...
#include "howso_costmap_2d/inflation_layer_interface.hpp"
...
class TestableAsymmetricInflationLayer : public howso_costmap_2d::AsymmetricInflationLayer
{
public:
...
TEST(AsymmetricInflationLayerInitTest, init_throws_on_negative_inflation_radius)
{
  ...
  auto node = std::make_shared<howso::LifecycleNode>("init_test_infl", "", options);
  auto tf = howso::create_transform_buffer(node);
  howso_costmap_2d::LayeredCostmap layers("map", false, false);
...
TEST(AsymmetricInflationLayerInitTest, init_throws_on_negative_left_scaling)
{
  ...
  auto node = std::make_shared<howso::LifecycleNode>("init_test_left", "", options);
  auto tf = howso::create_transform_buffer(node);
  howso_costmap_2d::LayeredCostmap layers("map", false, false);
  layers.resizeMap(10, 10, 0.1, 0.0, 0.0);
  auto layer = std::make_shared<TestableAsymmetricInflationLayer>();
...
TEST(AsymmetricInflationLayerInitTest, init_throws_on_negative_right_scaling)
{
  ...
  auto node = std::make_shared<howso::LifecycleNode>("init_test_right", "", options);
  auto tf = howso::create_transform_buffer(node);
  howso_costmap_2d::LayeredCostmap layers("map", false, false);
  layers.resizeMap(10, 10, 0.1, 0.0, 0.0);
  auto layer = std::make_shared<TestableAsymmetricInflationLayer>();
...
TEST(AsymmetricInflationLayerInitTest, init_throws_on_negative_goal_distance_threshold)
{
  ...
  auto node = std::make_shared<howso::LifecycleNode>("init_test_goal", "", options);
  auto tf = howso::create_transform_buffer(node);
  howso_costmap_2d::LayeredCostmap layers("map", false, false);
  layers.resizeMap(10, 10, 0.1, 0.0, 0.0);
  auto layer = std::make_shared<TestableAsymmetricInflationLayer>();
...
TEST(AsymmetricInflationLayerInitTest, init_throws_on_invalid_num_threads)
{
  ...
  auto node = std::make_shared<howso::LifecycleNode>("init_test_threads", "", options);
  auto tf = howso::create_transform_buffer(node);
  howso_costmap_2d::LayeredCostmap layers("map", false, false);
  layers.resizeMap(10, 10, 0.1, 0.0, 0.0);
  auto layer = std::make_shared<TestableAsymmetricInflationLayer>();
...
protected:
  void SetUp() override
  {
    ...
    tf_ = howso::create_transform_buffer(node_);

    // 40x40 cells, 0.1 m resolution, origin at (0,0) — world range [0, 4] m.
    layers_ = std::make_shared<howso_costmap_2d::LayeredCostmap>("map", false, false);
  ...
  {
    layers_->getCostmap()->setCost(mx, my, howso_costmap_2d::LETHAL_OBSTACLE);
  }
  ...
  std::shared_ptr<howso_costmap_2d::LayeredCostmap> layers_;
...
TEST_F(AsymmetricInflationIntegrationTest, no_path_produces_symmetric_output)
{
  ...
  EXPECT_GT(left, howso_costmap_2d::FREE_SPACE)
...
{
  std::shared_ptr<howso_costmap_2d::Layer> base_layer = layer_;
  auto inflation_layer =
    std::dynamic_pointer_cast<howso_costmap_2d::InflationLayerInterface>(base_layer);
...
TEST_F(AsymmetricInflationIntegrationTest, seed_distance_map_misses_obstacle_far_from_path)
{
  ...
  EXPECT_EQ(
    ...
    howso_costmap_2d::LETHAL_OBSTACLE)
