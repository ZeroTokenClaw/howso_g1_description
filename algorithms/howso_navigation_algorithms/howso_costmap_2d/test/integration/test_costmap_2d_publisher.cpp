...
class CostmapRosLifecycleNode : public howso::LifecycleNode
{
public:
  ...
  {
    costmap_ros_ = std::make_shared<howso_costmap_2d::Costmap2DROS>(
      name_,
      std::string{get_namespace()},
      get_parameter("use_sim_time").as_bool());
    costmap_thread_ = std::make_unique<howso::NodeThread>(costmap_ros_);
...
protected:
  ...
  std::unique_ptr<howso::NodeThread> costmap_thread_;
...
{
public:
  explicit LayerSubscriber(const howso::LifecycleNode::WeakPtr & parent)
  {
    ...
    layer_sub_ = node->create_subscription<howso_msgs::msg::Costmap>(
      topic_name,
      std::bind(&LayerSubscriber::layerCallback, this, std::placeholders::_1),
      howso::qos::LatchedSubscriptionQoS(3),
      callback_group_);
  ...
  std::promise<howso_msgs::msg::Costmap::ConstSharedPtr> layer_promise_;

protected:
  void layerCallback(const howso_msgs::msg::Costmap::ConstSharedPtr layer)
  {
  ...
  howso::Subscription<howso_msgs::msg::Costmap>::SharedPtr layer_sub_;
...
TEST_F(CostmapRosTestFixture, costmap_pub_test)
{
  ...
  unsigned int i = 0;
  for (; i < 7; ++i) {
    EXPECT_EQ(costmap_raw->data.at(i), howso_costmap_2d::FREE_SPACE);
  }
  for (; i < 10; ++i) {
    EXPECT_EQ(costmap_raw->data.at(i), howso_costmap_2d::LETHAL_OBSTACLE);
  }
  for (; i < 17; ++i) {
    EXPECT_EQ(costmap_raw->data.at(i), howso_costmap_2d::FREE_SPACE);
  }
