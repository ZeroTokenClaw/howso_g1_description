...
#include "howso_costmap_2d/costmap_topic_collision_checker.hpp"
#include "howso_costmap_2d/costmap_2d.hpp"
#include "howso_costmap_2d/layered_costmap.hpp"
#include "howso_costmap_2d/static_layer.hpp"
#include "howso_costmap_2d/inflation_layer.hpp"
#include "howso_costmap_2d/costmap_2d_publisher.hpp"
...
using howso_util::geometry_utils::orientationAroundZAxis;

class DummyCostmapSubscriber : public howso_costmap_2d::CostmapSubscriber
{
public:
  DummyCostmapSubscriber(
    howso::LifecycleNode::SharedPtr node,
    std::string & topic_name)
  : CostmapSubscriber(node, topic_name)
  {}

  void setCostmap(howso_msgs::msg::Costmap::SharedPtr msg)
  {
    costmap_msg_ = msg;
    costmap_ = std::make_shared<howso_costmap_2d::Costmap2D>(
...
class DummyFootprintSubscriber : public howso_costmap_2d::FootprintSubscriber
{
public:
  DummyFootprintSubscriber(
    howso::LifecycleNode::SharedPtr node,
    std::string & topic_name,
    howso::TransformBuffer & tf)
...
class TestCollisionChecker : public howso::LifecycleNode
{
public:
  ...
  howso::CallbackReturn
  on_configure(const rclcpp_lifecycle::State & /*state*/)
  {
    RCLCPP_INFO(get_logger(), "Configuring");
    callback_group_ = create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive, false);
    tf_buffer_ = howso::create_transform_buffer(shared_from_this());
    tf_listener_ = howso::create_transform_listener(*tf_buffer_, shared_from_this());
    tf_broadcaster_ = howso::create_transform_broadcaster(shared_from_this());
    ...
    collision_checker_ = std::make_unique<howso_costmap_2d::CostmapTopicCollisionChecker>(
      *costmap_sub_, *footprint_sub_, get_name());

    layers_ = new howso_costmap_2d::LayeredCostmap("map", false, false);
    // Add Static Layer
    ...
    executor_thread_ = std::make_unique<howso::NodeThread>(executor_);
  ...
  bool testPose(double x, double y, double theta)
  {
    ...
    pose.position.y = y;
    pose.position.z = 0.0;
    pose.orientation = howso_util::geometry_utils::orientationAroundZAxis(theta);
  ...
  void setFootprint(double footprint_padding, double robot_radius)
  {
    ...
    new_footprint = howso_costmap_2d::makeFootprintFromRadius(robot_radius);
    howso_costmap_2d::padFootprint(new_footprint, footprint_padding);
...
protected:
  ...
  void publishFootprint()
  {
    ...
    howso_costmap_2d::transformFootprint(x_, y_, yaw_, footprint_, oriented_footprint);
  ...
  void publishCostmap()
  {
    ...
    costmap_sub_->setCostmap(
      std::make_shared<howso_msgs::msg::Costmap>(toCostmapMsg(layers_->getCostmap())));
  }
  ...
  howso_msgs::msg::Costmap
  toCostmapMsg(howso_costmap_2d::Costmap2D * costmap)
  {
    ...
    howso_msgs::msg::Costmap costmap_msg;
  ...
  std::unique_ptr<howso::NodeThread> executor_thread_;

  std::shared_ptr<DummyCostmapSubscriber> costmap_sub_;
  std::shared_ptr<DummyFootprintSubscriber> footprint_sub_;
  std::unique_ptr<howso_costmap_2d::CostmapTopicCollisionChecker> collision_checker_;

  howso_costmap_2d::LayeredCostmap * layers_{nullptr};
