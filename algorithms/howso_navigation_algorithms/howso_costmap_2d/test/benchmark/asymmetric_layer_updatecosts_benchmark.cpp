...
#include "howso_costmap_2d/asymmetric_inflation_layer.hpp"
 ...
 */
class BenchmarkAsymmetricInflationLayer : public howso_costmap_2d::AsymmetricInflationLayer
{
public:
  ...
  unsigned int seed = kObstacleSeed)
{
  ...
  std::memset(costmap.getCharMap(), howso_costmap_2d::FREE_SPACE, total);
  ...
  for (unsigned int attempts = 0; occupied < target && attempts < 10000u; ++attempts) {
    ...
    for (unsigned int cy = ry; cy < ey && occupied < target; ++cy) {
      for (unsigned int cx = rx; cx < ex && occupied < target; ++cx) {
        if (costmap.getCost(cx, cy) != howso_costmap_2d::LETHAL_OBSTACLE) {
          costmap.setCost(cx, cy, howso_costmap_2d::LETHAL_OBSTACLE);
          ++occupied;
        }
      }
    }
  ...
  unsigned int ox, unsigned int oy, unsigned int side, unsigned int seed)
{
  ...
  for (unsigned int cy = oy; cy < ey; ++cy) {
    for (unsigned int cx = ox; cx < ex; ++cx) {
      costmap.setCost(cx, cy, howso_costmap_2d::FREE_SPACE);
    }
  }
...
public:
  void SetUp(benchmark::State & state) override
  {
    ...
    node_ = std::make_shared<howso::LifecycleNode>("inflation_benchmark", "", opts);

    layers_ = std::make_unique<howso_costmap_2d::LayeredCostmap>(kGlobalFrame, false, false);
    layers_->resizeMap(width_, height_, kResolution, 0.0, 0.0);

    layer_ = std::make_shared<howso_costmap_2d::InflationLayer>();
    layer_->initialize(layers_.get(), "inflation", nullptr, node_, nullptr);
    layers_->addPlugin(std::static_pointer_cast<howso_costmap_2d::Layer>(layer_));
  ...
  std::unique_ptr<howso_costmap_2d::LayeredCostmap> layers_;
  std::shared_ptr<howso_costmap_2d::InflationLayer> layer_;
};
...
public:
  void SetUp(benchmark::State & state) override
  {
    ...
    node_ = std::make_shared<howso::LifecycleNode>("legacy_inflation_benchmark", "", opts);

    layers_ = std::make_unique<howso_costmap_2d::LayeredCostmap>(kGlobalFrame, false, false);
    layers_->resizeMap(width_, height_, kResolution, 0.0, 0.0);

    layer_ = std::make_shared<howso_costmap_2d::LegacyInflationLayer>();
  ...
  howso::LifecycleNode::SharedPtr node_;
  std::unique_ptr<howso_costmap_2d::LayeredCostmap> layers_;
  std::shared_ptr<howso_costmap_2d::LegacyInflationLayer> layer_;
};
...
public:
  void SetUp(benchmark::State & state) override
  {
    ...
    node_ = std::make_shared<howso::LifecycleNode>("asymmetric_benchmark", "", opts);
    ...
    layers_->addPlugin(std::static_pointer_cast<howso_costmap_2d::Layer>(asym_layer_));
 ...
 * updateCosts() on a small moving patch — simulating an incoming obstacle
...
public:
  void SetUp(benchmark::State & state) override
  {
    ...
    node_ = std::make_shared<howso::LifecycleNode>("incremental_inflation_benchmark", "", opts);
...
public:
  void SetUp(benchmark::State & state) override
  {
    ...
    node_ =
      std::make_shared<howso::LifecycleNode>("incremental_legacy_inflation_benchmark", "", opts);
...
public:
  void SetUp(benchmark::State & state) override
  {
    ...
    node_ = std::make_shared<howso::LifecycleNode>("incremental_asymmetric_benchmark", "", opts);
