 ...
 */
class TestInflationLayer : public howso_costmap_2d::InflationLayer
{
public:
  ...
  unsigned int seed = 42)
{
  ...
  memset(master_array, howso_costmap_2d::FREE_SPACE, total_cells);
  ...
  while (occupied_count < target_occupied_cells && attempts < max_attempts) {
    ...
    for (unsigned int y = rect_y; y < end_y; ++y) {
      for (unsigned int x = rect_x; x < end_x; ++x) {
        ...
        if (current_cost != howso_costmap_2d::LETHAL_OBSTACLE) {
          costmap.setCost(x, y, howso_costmap_2d::LETHAL_OBSTACLE);
          occupied_count++;
...
bool saveCostmapAsPGM(
  const howso_costmap_2d::Costmap2D & costmap,
  ...
  const std::string & filename)
{
  ...
  for (unsigned int y = 0; y < height; ++y) {
    for (unsigned int x = 0; x < width; ++x) {
      ...
      if (cost == howso_costmap_2d::LETHAL_OBSTACLE) {
        // Lethal: black
        r = g = b = 0;
      } else if (cost == howso_costmap_2d::INSCRIBED_INFLATED_OBSTACLE) {
        // Inscribed: red
        r = 255; g = 0; b = 0;
      } else if (cost > howso_costmap_2d::FREE_SPACE &&  // NOLINT(readability/braces)
        cost < howso_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
      {
        // Inflated costs: gradient from blue (low) to yellow (high)
        float normalized = static_cast<float>(cost) /
          static_cast<float>(howso_costmap_2d::INSCRIBED_INFLATED_OBSTACLE);
        if (normalized < 0.5f) {
...
public:
  ...
  void SetUp(benchmark::State & state) override
  {
    ...
    // Initialize ROS node
    if (!node_) {
      node_ = std::make_shared<howso::LifecycleNode>("inflation_benchmark_node");
    }

    // Setup layered costmap
    layers_ = std::make_unique<howso_costmap_2d::LayeredCostmap>(global_frame, false, false);
  ...
  howso_costmap_2d::Costmap2D * master_costmap_;
};
...
public:
  ...
  {
    auto node = std::make_shared<howso::LifecycleNode>("custom_benchmark_node");
