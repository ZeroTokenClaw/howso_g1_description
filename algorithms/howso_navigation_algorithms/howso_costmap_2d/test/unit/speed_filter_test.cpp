...
#include "howso_util/occ_grid_values.hpp"
...
#include "howso_costmap_2d/costmap_filters/filter_values.hpp"
#include "howso_costmap_2d/costmap_filters/speed_filter.hpp"
...
public:
  ...
  {
    publisher_ = this->create_publisher<howso_msgs::msg::CostmapFilterInfo>(
      INFO_TOPIC, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());

    std::unique_ptr<howso_msgs::msg::CostmapFilterInfo> msg =
      std::make_unique<howso_msgs::msg::CostmapFilterInfo>();
    msg->type = type;
...
private:
  rclcpp::Publisher<howso_msgs::msg::CostmapFilterInfo>::SharedPtr publisher_;
};  // InfoPublisher
...
public:
  ...
  {
    subscriber_ = this->create_subscription<howso_msgs::msg::SpeedLimit>(
      speed_limit_topic, rclcpp::QoS(10),
      std::bind(&SpeedLimitSubscriber::speedLimitCallback, this, std::placeholders::_1));
  }

  void speedLimitCallback(
    const howso_msgs::msg::SpeedLimit::ConstSharedPtr msg)
  {
    msg_ = msg;
    speed_limit_updated_ = true;
  }

  howso_msgs::msg::SpeedLimit::ConstSharedPtr getSpeedLimit()
  {
    return msg_;
  }
...
private:
  howso::Subscription<howso_msgs::msg::SpeedLimit>::SharedPtr subscriber_;
  howso_msgs::msg::SpeedLimit::ConstSharedPtr msg_;
...
public:
  ...
  : width_(width), height_(height)
  {
    ...
    //   *----------------*
    // (0,0)
    data.resize(width_ * height_, howso_util::OCC_GRID_UNKNOWN);
...
protected:
  ...
  std::shared_ptr<howso_costmap_2d::SpeedFilter> speed_filter_;
  std::shared_ptr<howso_costmap_2d::Costmap2D> master_grid_;
...
private:
  ...
  void verifySpeedLimit(
    ...
    howso_msgs::msg::SpeedLimit::ConstSharedPtr speed_limit);
  howso_msgs::msg::SpeedLimit::ConstSharedPtr getSpeedLimit();
  howso_msgs::msg::SpeedLimit::ConstSharedPtr waitSpeedLimit();
  ...
  howso::TransformBuffer::SharedPtr tf_buffer_;
  howso::TransformListener::SharedPtr tf_listener_;
  howso::TransformBroadcaster::SharedPtr tf_broadcaster_;
...
void TestNode::createMaps(const std::string & mask_frame)
{
  ...
  // Create master_grid_
  master_grid_ = std::make_shared<howso_costmap_2d::Costmap2D>(
    width_ + 4, height_ + 4, resolution_, -2.0, -2.0, howso_costmap_2d::FREE_SPACE);
...
howso_msgs::msg::SpeedLimit::ConstSharedPtr TestNode::getSpeedLimit()
{
...
howso_msgs::msg::SpeedLimit::ConstSharedPtr TestNode::waitSpeedLimit()
{
...
{
  node_ = std::make_shared<howso::LifecycleNode>("test_node");
  tf_buffer_ = howso::create_transform_buffer(node_);
  tf_buffer_->setUsingDedicatedThread(true);  // One-thread broadcasting-listening model
  tf_listener_ = howso::create_transform_listener(*tf_buffer_, node_);

  howso_costmap_2d::LayeredCostmap layers(global_frame, false, false);
  ...
  speed_filter_ = std::make_shared<howso_costmap_2d::SpeedFilter>();
...
{
  tf_broadcaster_ = howso::create_transform_broadcaster(node_);
  ...
  howso_msgs::msg::SpeedLimit::ConstSharedPtr speed_limit)
{
  ...
  if (type == howso_costmap_2d::SPEED_FILTER_PERCENT) {
    if (expected_limit < 0.0 || expected_limit > 100.0) {
      expected_limit = howso_costmap_2d::NO_SPEED_LIMIT;
    }
  ...
  } else if (type == howso_costmap_2d::SPEED_FILTER_ABSOLUTE) {
  ...
  double tr_x, double tr_y)
{
  ...
  speed_limit = waitSpeedLimit();
  ASSERT_TRUE(speed_limit != nullptr);
  EXPECT_EQ(speed_limit->speed_limit, howso_costmap_2d::NO_SPEED_LIMIT);
...
void TestNode::testOutOfMask(uint8_t type, double base, double multiplier)
{
  ...
  geometry_msgs::msg::Pose pose;
  howso_msgs::msg::SpeedLimit::ConstSharedPtr old_speed_limit, speed_limit;
...
TEST_F(TestNode, testPercentSpeedLimit)
{
  ...
  createMaps("map");
  publishMaps(howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0);
  EXPECT_TRUE(createSpeedFilter("map"));

  // Test SpeedFilter
  testFullMask(howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0, NO_TRANSLATION, NO_TRANSLATION);
...
TEST_F(TestNode, testIncorrectPercentSpeedLimit)
{
  // Initialize test system
  createMaps("map");
  publishMaps(howso_costmap_2d::SPEED_FILTER_PERCENT, -50.0, 2.0);
  EXPECT_TRUE(createSpeedFilter("map"));

  // Test SpeedFilter
  testIncorrectLimits(howso_costmap_2d::SPEED_FILTER_PERCENT, -50.0, 2.0);
...
TEST_F(TestNode, testAbsoluteSpeedLimit)
{
  // Initialize test system
  createMaps("map");
  publishMaps(howso_costmap_2d::SPEED_FILTER_ABSOLUTE, 1.23, 4.5);
  EXPECT_TRUE(createSpeedFilter("map"));

  // Test SpeedFilter
  testFullMask(howso_costmap_2d::SPEED_FILTER_ABSOLUTE, 1.23, 4.5, NO_TRANSLATION, NO_TRANSLATION);
...
TEST_F(TestNode, testIncorrectAbsoluteSpeedLimit)
{
  // Initialize test system
  createMaps("map");
  publishMaps(howso_costmap_2d::SPEED_FILTER_ABSOLUTE, -50.0, 2.0);
  EXPECT_TRUE(createSpeedFilter("map"));

  // Test SpeedFilter
  testIncorrectLimits(howso_costmap_2d::SPEED_FILTER_ABSOLUTE, -50.0, 2.0);
...
TEST_F(TestNode, testOutOfBounds)
{
  ...
  // Test SpeedFilter
  testOutOfMask(howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0);
...
TEST_F(TestNode, testInfoRePublish)
{
  ...
  rePublishInfo(howso_costmap_2d::SPEED_FILTER_PERCENT, 0.1, 0.2);

  // Test SpeedFilter
  testSimpleMask(
    howso_costmap_2d::SPEED_FILTER_PERCENT, 0.1, 0.2, NO_TRANSLATION, NO_TRANSLATION);
...
TEST_F(TestNode, testMaskRePublish)
{
  ...
  // Test SpeedFilter
  testSimpleMask(
    howso_costmap_2d::SPEED_FILTER_ABSOLUTE, 1.23, 4.5, NO_TRANSLATION, NO_TRANSLATION);
...
TEST_F(TestNode, testDifferentFrame)
{
  ...
  // Test SpeedFilter
  testFullMask(howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0, TRANSLATION_X, TRANSLATION_Y);
...
TEST_F(TestNode, testPathLookaheadDetectsZoneAhead)
{
  ...
  testPathLookaheadDetection(howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0, 1.0, NO_TRANSLATION,
    NO_TRANSLATION);
...
TEST_F(TestNode, testPathLookaheadFallBackToRobotPose)
{
  ...
  testSimpleMask(
    howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0, NO_TRANSLATION, NO_TRANSLATION);
...
TEST_F(TestNode, testPathLookaheadWithDifferentFrame)
{
  ...
  testPathLookaheadDetection(howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0, 1.0, TRANSLATION_X,
    TRANSLATION_Y);
...
TEST_F(TestNode, testPathLookaheadInvalidDecel)
{
  ...
  testPathLookaheadDetection(
    howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0,
...
TEST_F(TestNode, testPathLookaheadInvalidMinLookahead)
{
  ...
  testPathLookaheadDetection(
    howso_costmap_2d::SPEED_FILTER_PERCENT, 0.0, 1.0, 1.0,
