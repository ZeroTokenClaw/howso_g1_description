...
void TestNode::createMaps()
{
  ...
  for (unsigned int i = 0; i < width * height; i++) {
    data = i * 10;
    if (data <= howso_util::OCC_GRID_OCCUPIED) {
      occ_grid_->data[i] = data;
    } else {
      occ_grid_->data[i] = howso_util::OCC_GRID_UNKNOWN;
    }
  }

  // Create costmap_ (convert OccupancyGrid -> to Costmap2D)
  costmap_ = std::make_shared<howso_costmap_2d::Costmap2D>(*occ_grid_);
}
...
void TestNode::verifyCostmap()
{
  ...
  for (it = 0; it < (costmap_->getSizeInCellsX() * costmap_->getSizeInCellsY() - 1); it++) {
    ...
    if (it != costmap_->getSizeInCellsX() * costmap_->getSizeInCellsY() - 1) {
      data_ref = std::round(
        static_cast<double>(howso_costmap_2d::LETHAL_OBSTACLE - howso_costmap_2d::FREE_SPACE) * it /
        10);
    } else {
      data_ref = howso_costmap_2d::NO_INFORMATION;
    }
