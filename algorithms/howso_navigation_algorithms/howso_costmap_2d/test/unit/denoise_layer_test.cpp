...
#include "howso_costmap_2d/denoise_layer.hpp"
...
{
/**
 * @brief howso_costmap_2d::DenoiseLayer class wrapper
 *
...
public:
  ...
  static void initialize(howso_costmap_2d::DenoiseLayer & d)
  {
    d.onInitialize();
  }

  static void configure(
    howso_costmap_2d::DenoiseLayer & d, ConnectivityType connectivity, size_t minimal_group_size)
  {
    d.enabled_ = true;
  ...
  static std::tuple<bool, ConnectivityType, size_t> getParameters(
    const howso_costmap_2d::DenoiseLayer & d)
  {
...
private:
  howso_costmap_2d::DenoiseLayer denoise_;
};
...
TEST_F(DenoiseLayerTester, constructorAndDestructor) {
  ASSERT_NO_THROW(
  {
    howso_costmap_2d::DenoiseLayer layer;
  });
}
...
TEST_F(DenoiseLayerTester, updateCostsIfDisabled) {
  howso_costmap_2d::DenoiseLayer layer;
  howso_costmap_2d::Costmap2D costmap(1, 1, 1., 0., 0., 255);
...
TEST_F(DenoiseLayerTester, updateCosts) {
  howso_costmap_2d::DenoiseLayer layer;
  howso_costmap_2d::Costmap2D costmap(1, 1, 1., 0., 0.);
...
std::shared_ptr<howso_costmap_2d::DenoiseLayer> constructLayer(
  howso::LifecycleNode::SharedPtr node =
  std::make_shared<howso::LifecycleNode>("test_node"))
{
  auto tf = howso::create_transform_buffer(node);
  auto layers = std::make_shared<howso_costmap_2d::LayeredCostmap>("frame", false, false);

  auto deleter = [node, tf, layers](howso_costmap_2d::DenoiseLayer * ptr)
    {
      delete ptr;
    };
  auto layer = std::shared_ptr<howso_costmap_2d::DenoiseLayer>(
    new howso_costmap_2d::DenoiseLayer, deleter);
...
TEST_F(DenoiseLayerTester, initializeCustom) {
  auto node = std::make_shared<howso::LifecycleNode>("test_node");
