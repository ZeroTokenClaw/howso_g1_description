...
TEST(mapToWorldNoBounds, MapToWorldNoBoundsNegativeMapCoords)
{
  ...
  std::unique_ptr<howso_costmap_2d::Costmap2D> map;

  map = std::make_unique<howso_costmap_2d::Costmap2D>(10, 10, 1.0, 0.0, 0.0);
  map->mapToWorldNoBounds(-1, -1, wx, wy);
  EXPECT_DOUBLE_EQ(wx, -0.5);
  EXPECT_DOUBLE_EQ(wy, -0.5);

  map = std::make_unique<howso_costmap_2d::Costmap2D>(10, 10, 1.0, 1.0, 2.0);
  map->mapToWorldNoBounds(-5, -5, wx, wy);
  EXPECT_DOUBLE_EQ(wx, -3.5);
  EXPECT_DOUBLE_EQ(wy, -2.5);

  map = std::make_unique<howso_costmap_2d::Costmap2D>(10, 10, 2.0, 3.0, 4.0);
