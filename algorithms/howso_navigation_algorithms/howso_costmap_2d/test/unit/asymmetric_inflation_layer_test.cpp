...
using howso_costmap_2d::TestableAsymmetricInflationLayer;
using howso_costmap_2d::Side;
...
TEST(DisfavoredLutTest, lut_uses_c_side_scaling_factor)
{
    ...
    d_scaled += lut_precision)
  {
    ...
    unsigned char expected =
      static_cast<unsigned char>((howso_costmap_2d::INSCRIBED_INFLATED_OBSTACLE - 1) * factor);
  ...
  for (int d_scaled = 1; d_scaled < lut_precision * 3; ++d_scaled) {
    EXPECT_EQ(
      ...
      howso_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
      << "Expected INSCRIBED at d_scaled=" << d_scaled;
  }

  // Distance 0 must be LETHAL_OBSTACLE.
  EXPECT_EQ(layer->cost_lut_disfavored_[0], howso_costmap_2d::LETHAL_OBSTACLE);
}
...
{
  std::vector<howso_costmap_2d::AsymmetricPathSegment> segs = {
    {{0.0, 0.5}, {3.0, 0.5}}
  };
...
protected:
  void SetUp() override
  {
    ...
    costmap_ = std::make_unique<howso_costmap_2d::Costmap2D>(40, 40, 0.1, 0.0, 0.0);
    layer_ = std::make_unique<TestableAsymmetricInflationLayer>();
  ...
  std::vector<howso_costmap_2d::AsymmetricPathSegment> makeSegments(
    const std::vector<std::pair<double, double>> & path)
  {
    std::vector<howso_costmap_2d::AsymmetricPathSegment> segments;
  ...
  std::unordered_map<uint64_t, std::vector<size_t>> buildSpatialHash(
    const std::vector<howso_costmap_2d::AsymmetricPathSegment> & path_segments)
  {
  ...
  std::unique_ptr<howso_costmap_2d::Costmap2D> costmap_;
  std::unique_ptr<TestableAsymmetricInflationLayer> layer_;
};
...
{
  std::vector<howso_costmap_2d::AsymmetricPathSegment> path_segments = {
    {{0.5, 2.0}, {1.0, 2.0}},
    {{3.0, 2.0}, {3.5, 2.0}}
  };
