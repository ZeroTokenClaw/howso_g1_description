...
protected:
  std::shared_ptr<howso_costmap_2d::ObstacleLayer> obstacle_layer_;

private:
  std::shared_ptr<TestLifecycleNode> node_;
  howso_costmap_2d::LayeredCostmap layers_;
};
...
TEST_F(ObstacleLayerTest, testPointWithinCellMaxRange)
{
  ...
  ASSERT_EQ(cost, howso_costmap_2d::LETHAL_OBSTACLE);
}
...
TEST_F(ObstacleLayerTest, testPointBeyondCellMaxRange)
{
  ...
  ASSERT_NE(cost, howso_costmap_2d::LETHAL_OBSTACLE);
}
...
TEST_F(ObstacleLayerTest, testRaytraceClearsPathAndMarksEndpoint)
{
  ...
  for (unsigned int x = 0; x < 4; ++x) {
    obstacle_layer_->setCost(x, 0, howso_costmap_2d::LETHAL_OBSTACLE);
  }
...
TEST_F(ObstacleLayerTest, testDiagonalRaytraceDoesNotClearEndpoint)
{
  ...
  obstacle_layer_->setCost(mx_end, my_end, howso_costmap_2d::LETHAL_OBSTACLE);
  // Mark a cell along the diagonal path
  unsigned int mx_mid, my_mid;
  obstacle_layer_->worldToMap(0.45, 0.45, mx_mid, my_mid);
  obstacle_layer_->setCost(mx_mid, my_mid, howso_costmap_2d::LETHAL_OBSTACLE);
  ...
  ASSERT_EQ(cost_mid, howso_costmap_2d::FREE_SPACE);
}
...
TEST_F(ObstacleLayerTest, testClearDiagonalDistance) {
  ...
  for (unsigned int x = 0; x < obstacle_layer_->getSizeInCellsX(); x++) {
    for (unsigned int y = 0; y < obstacle_layer_->getSizeInCellsY(); y++) {
      obstacle_layer_->setCost(x, y, howso_costmap_2d::LETHAL_OBSTACLE);
    }
  }
  ...
  for (unsigned int i = 0; i < 8; i++) {
    ASSERT_EQ(obstacle_layer_->getCost(i, i), howso_costmap_2d::FREE_SPACE);
  }
  ASSERT_EQ(countValues(*obstacle_layer_, howso_costmap_2d::FREE_SPACE), 8);
  ASSERT_EQ(countValues(*obstacle_layer_, howso_costmap_2d::LETHAL_OBSTACLE),
            20 * 20 - 8);
}
