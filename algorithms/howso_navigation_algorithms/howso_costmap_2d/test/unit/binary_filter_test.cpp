...
#include "howso_costmap_2d/costmap_filters/binary_filter.hpp"
...
private:
  howso::Subscription<std_msgs::msg::Bool>::SharedPtr subscriber_;
...
public:
  ...
  : width_(width), height_(height)
  {
    ...
    unsigned int mx, my;
    data[0] = howso_util::OCC_GRID_UNKNOWN;
    for (mx = 1; mx < width_; mx++) {
      data[mx] = howso_util::OCC_GRID_FREE;
    }
    unsigned int it;
...
protected:
  ...
  std::shared_ptr<howso_costmap_2d::BinaryFilter> binary_filter_;
...
bool TestNode::createBinaryFilter(const std::string & global_frame, double flip_threshold)
{
  ...
  binary_filter_ = std::make_shared<howso_costmap_2d::BinaryFilter>();
  ...
  double base, double multiplier, double flip_threshold, double tr_x, double tr_y)
{
  ...
  pose.position.z = 0.0;
  pose.orientation = howso_util::geometry_utils::orientationAroundZAxis(0.0);
  publishTransform();
...
TEST_F(TestNode, testBinaryState)
{
  ...
  createMaps("map");
  publishMaps(howso_costmap_2d::BINARY_FILTER, MASK_TOPIC, 0.0, 1.0);
...
TEST_F(TestNode, testBinaryStateScaled)
{
  // Initialize test system
  createMaps("map");
  publishMaps(howso_costmap_2d::BINARY_FILTER, MASK_TOPIC, 100.0, -1.0);
...
TEST_F(TestNode, testInfoRePublish)
{
  ...
  publishMaps(howso_costmap_2d::BINARY_FILTER, "dummy_topic", 0.0, 1.0);
  ...
  rePublishInfo(howso_costmap_2d::BINARY_FILTER, MASK_TOPIC, 0.0, 1.0);
