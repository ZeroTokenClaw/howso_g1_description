...
using ClearEntireCostMapSrv = howso_msgs::srv::ClearEntireCostmap;
using ClearCostmapAroundPoseSrv = howso_msgs::srv::ClearCostmapAroundPose;
using ClearCostmapAroundRobotSrv = howso_msgs::srv::ClearCostmapAroundRobot;
using ClearCostmapExceptRegionSrv = howso_msgs::srv::ClearCostmapExceptRegion;
...
protected:
  void SetUp() override
  {
    costmap_ = std::make_shared<howso_costmap_2d::Costmap2DROS>("costmap");

    // Send identity transform to ensure costmap can transform robot poses during service calls
    auto tf_broadcaster = howso::create_static_transform_broadcaster(costmap_);
...
private:
  ...
  std::shared_ptr<howso_costmap_2d::Costmap2DROS> costmap_;
  howso::ServiceClient<ServiceT>::SharedPtr clear_costmap_client_;
