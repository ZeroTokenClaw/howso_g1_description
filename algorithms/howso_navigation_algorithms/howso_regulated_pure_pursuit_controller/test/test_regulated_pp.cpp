...
#include "howso_regulated_pure_pursuit_controller/regulated_pure_pursuit_controller.hpp"
#include "howso_costmap_2d/costmap_filters/filter_values.hpp"
#include "howso_core/controller_exceptions.hpp"
#include "geometry_msgs/msg/transform_stamped.hpp"
#include "howso_controller/plugins/feasible_path_handler.hpp"
#include "howso_controller/plugins/simple_goal_checker.hpp"
#include "howso_ros_common/tf2_factories.hpp"

class BasicAPIRPP : public howso_regulated_pure_pursuit_controller::RegulatedPurePursuitController
{
public:
  BasicAPIRPP()
  : howso_regulated_pure_pursuit_controller::RegulatedPurePursuitController() {}
...
{
  auto node = std::make_shared<howso::LifecycleNode>("testRPP");
  std::string name = "PathFollower";
  auto tf = howso::create_transform_buffer(node);
  auto costmap = std::make_shared<howso_costmap_2d::Costmap2DROS>("fake_costmap");
  ...
  ctrl->setSpeedLimit(howso_costmap_2d::NO_SPEED_LIMIT, false);
  EXPECT_EQ(ctrl->getSpeed(), base_speed);
  ctrl->setSpeedLimit(30, true);
  EXPECT_EQ(ctrl->getSpeed(), base_speed * 0.3);
  ctrl->setSpeedLimit(howso_costmap_2d::NO_SPEED_LIMIT, true);
  EXPECT_EQ(ctrl->getSpeed(), base_speed);
}
...
TEST(RegulatedPurePursuitTest, rotateTests)
{
  ...
  howso_controller::SimpleGoalChecker goal_checker;
  ...
  howso_controller::SimpleGoalChecker stateful_goal_checker;
...
{
  auto node = std::make_shared<howso::LifecycleNode>("Smactest");
  auto costmap = std::make_shared<howso_costmap_2d::Costmap2DROS>("global_costmap");
  costmap->on_configure(rclcpp_lifecycle::State());
  auto ctrl =
    std::make_unique<howso_regulated_pure_pursuit_controller::RegulatedPurePursuitController>();
  auto tf = howso::create_transform_buffer(node);
...
TEST(RegulatedPurePursuitTest, computeVelocityByDWPP)
{
  ...
  ctrl->configure(node, name, tf, costmap);
  ctrl->activate();
  howso_controller::FeasiblePathHandler path_handler;
  ...
  howso_controller::SimpleGoalChecker checker;
...
TEST(RegulatedPurePursuitTest, testObstacleBeyondGoal)
{
  ...
  raw_costmap->setCost(obs_mx, obs_my, howso_costmap_2d::LETHAL_OBSTACLE);
  raw_costmap->setCost(obs_mx, obs_my + 1, howso_costmap_2d::LETHAL_OBSTACLE);
  raw_costmap->setCost(obs_mx + 1, obs_my, howso_costmap_2d::LETHAL_OBSTACLE);
  raw_costmap->setCost(obs_mx + 1, obs_my + 1, howso_costmap_2d::LETHAL_OBSTACLE);
