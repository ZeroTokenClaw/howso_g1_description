  ...
  Parameters * params)
{
  ...
  footprint_collision_checker_ = std::make_unique<howso_costmap_2d::
      FootprintCollisionChecker<howso_costmap_2d::Costmap2D *>>(costmap_);
  ...
  const double & carrot_dist, const double & dist_to_path_end)
{
  ...
  while (i * projection_time < max_allowed_time_to_collision_check) {
    ...
    curr_pose.orientation = howso_util::geometry_utils::orientationAroundZAxis(theta);
