...
#include "howso_util/controller_utils.hpp"
...
using std::max;
using std::abs;
using namespace howso_costmap_2d;  // NOLINT
  ...
  std::shared_ptr<howso_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  node_ = parent;
  if (!node) {
    throw howso_core::ControllerException("Unable to lock node!");
  }
  ...
  const geometry_msgs::msg::PoseStamped & global_goal)
{
  ...
  howso_costmap_2d::Costmap2D * costmap = costmap_ros_->getCostmap();
  std::unique_lock<howso_costmap_2d::Costmap2D::mutex_t> lock(*(costmap->getMutex()));

  // Transform the plan from costmap's global frame to robot base frame
  nav_msgs::msg::Path transformed_plan;
  if (!howso_util::transformPathInTargetFrame(
      transformed_global_plan, transformed_plan, *tf_,
      costmap_ros_->getBaseFrameID(), costmap_ros_->getTransformTolerance()))
  {
    throw howso_core::ControllerTFError(
  ...
  auto carrot_pose = howso_util::getLookAheadPoint(lookahead_dist, transformed_plan);
  ...
  if (params_->use_fixed_curvature_lookahead) {
    auto curvature_lookahead_pose = howso_util::getLookAheadPoint(
      curv_lookahead_dist,
  ...
  const double dist_to_path_end =
    howso_util::geometry_utils::calculate_path_length(transformed_plan);
  ...
  if (params_->use_collision_detection &&
    ...
    dist_to_path_end))
  {
    throw howso_core::NoValidControl("RegulatedPurePursuitController detected collision ahead!");
  }
  ...
  const bool & percentage)
{
  ...
  if (speed_limit == howso_costmap_2d::NO_SPEED_LIMIT) {
