...
#include "howso_navfn_planner/navfn_planner.hpp"
...
#include "howso_navfn_planner/navfn.hpp"
#include "howso_util/costmap.hpp"
  ...
  std::function<bool()> cancel_checker)
{
  ...
  if (params_->tolerance == 0 && costmap_->getCost(mx_goal,
      my_goal) == howso_costmap_2d::LETHAL_OBSTACLE)
  {
  ...
  if (!makePlan(start.pose, goal.pose, params_->tolerance, cancel_checker, path)) {
    throw howso_core::NoValidPathCouldBeFound(
  ...
  nav_msgs::msg::Path & plan)
{
  ...
  std::unique_lock<howso_costmap_2d::Costmap2D::mutex_t> lock(*(costmap_->getMutex()));
...
NavfnPlanner::clearRobotCell(unsigned int mx, unsigned int my)
{
  ...
  costmap_->setCost(mx, my, howso_costmap_2d::FREE_SPACE);
}

}  // namespace howso_navfn_planner

#include "pluginlib/class_list_macros.hpp"
PLUGINLIB_EXPORT_CLASS(howso_navfn_planner::NavfnPlanner, howso_core::GlobalPlanner)
